...
def create(a,b,c,frame1,root,k):
    try:
        ...
        assert int(b)!=int and int(b)!=0,'Invalid entry'
        assert len(c)!=0,'No contact no. found'
        assert int(c)!=int and int(c)!=0,'Invalid contact no.'
    except:
...
def history():
    try:
        ...
        Button(toolbar,text=("Average Bill",round(df["Bill"].mean(),2)),font=("Courier", 10, 'bold')).pack()
